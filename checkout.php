...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $address        = htmlspecialchars($_POST['address']) . ", " . htmlspecialchars($_POST['city']) . ", " . htmlspecialchars($_POST['zip']);
    ...
    if ($stmt->execute()) {
        ...
        // B. (Optional) Insert items into 'order_items' table
        // Ideally, you should have a table: order_items (id, order_id, product_name, price, quantity)
        // This loop simulates that save.
        foreach ($_SESSION['cart'] as $item) {
            $p_name = $item['name'];
            $p_price = $item['price'];
            $p_qty = $item['quantity'];
            
            // Uncomment this if you have the table created:
...
<head>
    ...
    <style>
        ...
        body { background-color: #f4f6f9; font-family: 'Segoe UI', sans-serif; padding-top: 40px; padding-bottom: 40px; }
        ...
        .btn-pay { background: wheat; color: white; width: 100%; padding: 15px; border: none; border-radius: 8px; font-weight: 800; font-size: 1rem; transition: 0.3s; margin-top: 20px; }
        .btn-pay:hover { background: wheat; color: black; transform: translateY(-2px); }
                        ...
                        <form id="checkoutForm" method="POST" onsubmit="return startLoading()">
                            ...
                            <div class="row g-3 mb-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="int" name="zip" class="form-control" required>
                                </div>
                            </div>
